						...
						<li class="active"><a href="/archives">Archives</a>
							<div class="dropdown-content">
								<ul id="category">
									...
									<li class="active"><a href="archives/jsp">JSP</a></li>
								</ul>
							</div>
						</li>
...
<div class="text-center">
	<ul class="pagination">
		<c:if test="${pageutil.prev }">
			<li><a href="jsp${pageutil.makeQuery(pageutil.startPage - 1) }">&laquo;</a></li>
		</c:if>

		<c:forEach begin="${pageutil.startPage }" end="${pageutil.endPage }" var="idx">
			<li
				<c:out value="${pageutil.cri.page == idx?'class = active ' : '' }"/>>
				<a href="jsp${pageutil.makeQuery(idx) }">${idx }</a>
			</li>
		</c:forEach>

		<c:if test="${pageutil.next && pageutil.endPage > 0}">
			<li><a href="jsp${pageutil.makeQuery(pageutil.endPage + 1) }">&raquo;</a></li>
		</c:if>
	</ul>
</div>
